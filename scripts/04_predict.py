...
def find_latest_best_model(training_root: Path) -> Path:
    # Primary location: <project>/training_runs/*/weights/best.pt
    best_models = list(training_root.glob("*/weights/best.pt"))

    # Alternate location used by Ultralytics: runs/detect/training_runs/*/weights/best.pt
    alt_root = Path("runs") / "detect" / training_root
    if alt_root.exists():
        best_models += list(alt_root.glob("*/weights/best.pt"))

    # Fallback: search recursively for any training_runs/*/weights/best.pt
    if not best_models:
        best_models = list(Path(".").glob("**/" + str(training_root) + "/*/weights/best.pt"))

    if not best_models:
        raise FileNotFoundError(
            f"No best.pt model found inside: {training_root} (also checked runs/detect and recursive locations)"
        )
...
def main() -> None:
    ...
    # Ensure output folder exists; use absolute path so Ultralytics does not place
    # results under `runs/detect/...` when a relative project is passed.
    OUTPUT_PROJECT.mkdir(parents=True, exist_ok=True)
    project_path = OUTPUT_PROJECT.resolve()

    print("========== YOLO TEST PREDICTION ==========")
    print(f"Model:       {model_path}")
    print(f"Test folder: {TEST_IMAGES_DIR}")
    print(f"Images:      {len(test_images)}")
    print(f"Output root: {project_path}")
    ...
    # Pass an absolute project path so outputs are written to `prediction_outputs/<name>`
